<template>
  <div
    v-if="store.state.isCoverageLoading"
  ...
  <div
    v-else-if="!store.state.coverageReport || store.state.coverageReport.files.length === 0"
    class="text-gray-500 text-center pt-10"
  >
  ...
  <div v-else>
    ...
    <div class="bg-gray-800 rounded-lg shadow-lg overflow-hidden">
      ...
      <div class="divide-y divide-gray-700">
        <div
          v-for="file in store.state.coverageReport.files"
          :key="file.path"
          class="hover:bg-gray-700/50 cursor-pointer"
          @click="showFileCoverage(file.path)"
        >
          <div class="flex items-center p-3">
            ...
            <div class="w-28 text-right flex-shrink-0 pr-3">
              <span
                class="text-sm"
                :class="{'text-green-400': file.coverage_percent > 80, 'text-yellow-400': file.coverage_percent > 50 && file.coverage_percent <= 80, 'text-red-400': file.coverage_percent <= 50}"
              >{{ file.coverage_percent.toFixed(2) }}%</span>
            </div>
          </div>
        </div>
      </div>
