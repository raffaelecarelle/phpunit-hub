<template>
  <div class="bg-gray-800 rounded-lg shadow-lg p-4">
    ...
    <div class="flex justify-center flex-wrap gap-x-4 gap-y-2 text-sm">
      ...
      <span
        v-if="results?.summary.failures > 0"
      ...
      <span
        v-if="results?.summary.warnings > 0 && store.state.options.displayWarnings"
        class="flex items-center"
      >
      ...
      <span
        v-if="results?.summary.skipped > 0 && store.state.options.displaySkipped"
        class="flex items-center"
      >
        <span class="w-2 h-2 rounded-full bg-gray-500 mr-2" />
        {{ results?.summary.skipped }} Skipped
      </span>
      <span
        v-if="results?.summary.deprecations > 0 && store.state.options.displayDeprecations"
        class="flex items-center"
      >
        <span class="w-2 h-2 rounded-full bg-purple-500 mr-2" />
        {{ results?.summary.deprecations }} Deprecations
      </span>
      <span
        v-if="results?.summary.notices > 0 && store.state.options.displayNotices"
        class="flex items-center"
      >
        <span class="w-2 h-2 rounded-full bg-sky-500 mr-2" />
        {{ results?.summary.notices }} Notices
      </span>
      <span
        v-if="results?.summary.incomplete > 0 && store.state.options.displayIncomplete"
        class="flex items-center"
      >
      ...
      <span
        v-if="results?.summary.risky > 0 && store.state.options.displayRisky"
        class="flex items-center"
      >
        <span class="w-2 h-2 rounded-full bg-orange-500 mr-2" />
...
const { getStatusCounts } = useTestResults(results);
