<template>
    <div class="bg-gray-800 rounded-lg shadow-lg p-4 mb-4">
        <button @click="goBackToCoverageReport" class="bg-gray-600 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded transition duration-150 ease-in-out mb-4">
        ...
        <pre class="bg-gray-900 p-4 rounded-lg overflow-x-auto text-sm font-mono">
            <div v-for="line in store.state.fileCoverage.lines"
                ...
                <span class="text-gray-500 flex-shrink-0 text-right pr-3" style="user-select: none;">{{ line.number }}</span>
                <span style="flex: 1; white-space: pre;">
                    <span v-for="item in line.tokens" :key="item.value" :class="getTokenClass(item.type)">{{ item.value }}</span>
                </span>
            </div>
        </pre>
    </div>
