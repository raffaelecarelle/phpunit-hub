...
import { ref, watch } from 'https://cdn.jsdelivr.net/npm/vue@3/dist/vue.esm-browser.prod.js';

export const TestSearchBar = {
    props: ['store'],
    emits: ['update:filtered-suites'],
    setup(props, { emit }) {
        const searchQuery = ref('');

        const filterSuites = (query) => {
            if (!query) {
                emit('update:filtered-suites', props.store.state.testSuites);
                return;
            }

            const lowerCaseQuery = query.toLowerCase();
            const filtered = props.store.state.testSuites.map(suite => {
        ...
        watch(() => props.store.state.testSuites, (newSuites) => {
            searchQuery.value = '';
